...
Résultat :
On rentre dans le conteneur manager
...
Résultat:
On voit bien notre cluster avec le manager et tous les nodes
...
Commentaire:
J'utilise l'option scale afin de lancer 3 containers node.
Le docker ps me confirme que les 4 containers sont bien lancés.
...
Commentaire:
On voit bien que tous les noeuds sont bien créés
...
Commentaire:
Le nouveau noeud est bien créé
...
Commentaire:
On voit bien que le nouveau noeud fait bien parti du cluster

--------------------

Question 2
Pour utiliser Ansible sur des VMs / VPS, il faudrait modifier dans le fichier d'inventaire les noms des container en les remplaçant par les IP des machines. On aurai plus besoin du ansible_connection car on utiliserai le ssh.
Côté playbook, il faudrait modifier l'host dans la commande qui permet au worker de rejoindre le cluster. On pourrais utiliser l'ip de la machine.

-----------------

Question 3
Pour rendre ansible utilisable, j'ai créé un dossier ansible dans lequel j'ai réutilisé le playbook et le fichier inventory du repo précédent en adaptant le nom des machines dans le inventory.
J'ai modifié le docker composant en pointant pour les services manager et neoud vers le dockerfile que j'ai récupéré du repo précédent afin de lancer sur la bonne image avec Python installé dans le conteneur.
...
Commentaire:
On voit bien que le network est créé.
...
Question 3
Après avoir mis à jour le fichier hosts, j'accède bien à trafik et whoami depuis mon navigateur
TODO : Mettre des screens
...
Dans le stack je met à jour la partie labels des deploy vote et result. J'ajoute également le network "web" avec l'attribut "external" à true.
...
Commentaires:
On vois bien que le stack est deployée et que mon app de vote et mon app de résultats sont bien accèsssible dans mon navigateur et sont visibles dans traefik.
...
Commentaire:
Dans la stack, j'ajoute le network web et les labels traefik vers le port 9000.

Question 3
TODO: Mettre screen portainer
...
Question 2
On ajoute une stack manullement dans portainer en utilisant le contenu de la stack qu'on colle dans l'interface portainer 
